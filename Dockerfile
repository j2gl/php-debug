...
RUN echo '[Xdebug]' >> /usr/local/etc/php/php.ini && \
    ...
    # echo 'xdebug.remote_log=/var/www/html/xdebug-container.log' >> /usr/local/etc/php/php.ini && \
    # Need to set dockerhost to your ip machine 
    echo 'xdebug.remote_host=dockerhost' >> /usr/local/etc/php/php.ini 
